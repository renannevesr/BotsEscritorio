...
try:
   # driver.find_element(By.XPATH, '// *[ @ id = "topMenu"] / span[2]').click()
    ...
    while idx < rows:
        ...
        url2 =f'https://promobank.online/sistema/consulta/processador.php?tipo=2&value={beneficio}&getMatriculas=false&competencia=&modoConsulta=folha_atual&tipoCampanha=&isJson=false&_=1736881373532'
        driver.get(url2)
        time.sleep(3)
        ...
        for variavel in nomes_variaveis:
            try:
                if variavel in ['telefone', 'situacao']:
                    print("entrou")
                    dados = driver.find_element(By.XPATH, f'{eval(variavel)}').text
                    print("dados= ", dados)
                    dados1 = driver.find_element(By.XPATH, f'{eval(variavel)}')
                    print("dados1= ", dados1)
...
df.to_csv("especie23-29RJCENTRO2.csv")
